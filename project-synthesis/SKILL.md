...
在写 summary 和分析前，**必须先努力区分内容来源**，至少分为以下三类：
...
- 不要把用户团队在投前会、复盘会、内部讨论里对项目的**复述、总结、判断、追问**写成创始团队原话。
- 不要把第三方 reference（前同事、朋友、投资人、熟人评价）直接写成创始团队自己的想法变化，除非材料明确是创始人本人自述。
- 若某段材料只能看出“在讨论项目”，但无法可靠判断是谁提出的观点，必须明确标注“归因不确定”或用更保守的转述，不要硬猜。
- 若整个会议明显是第三方访谈或他人评价，应明确写成 `第三方评价` 或 `外部观察`，不要写成“团队认为”。

分析时的强约束：

- `过程中我们最关心的问题`：**只使用用户 / 用户团队提出、关心、追问或反复讨论的问题**。
- `创始团队在过程中想法的变化，体现的优缺点`：**只使用创始人 / 创始团队自己的自述、内部讨论、直连纪要或能可靠归因给他们的内容**。
- 如果要写“双方的错位点 / 张力 / 分歧”，必须明确区分为 `创始团队视角` 与 `我方团队视角`，不要混写。
...
- 摘要：
    ...
    - 若材料中同时存在创始团队与用户团队的内容，必须在展开时显式区分 `创始团队视角` 与 `我方团队视角`
    - 若材料主要来自第三方访谈或旁证，应用 `第三方评价` / `外部观察` 标注，而不是伪装成创始团队自述
...
- `过程中我们最关心的问题` 只能总结用户 / 用户团队的判断、担忧、追问和验证重点。
- `创始团队在过程中想法的变化，体现的优缺点` 只能总结创始团队自身材料里的变化，不要混入用户团队的投前整理、会议复盘或第三方评价。
- 若需要引用第三方 reference，应单独标注为 `第三方评价`、`外部参考` 或放进 founder/profile 风险说明，不要冒充为创始团队的自我认知。
...
若材料中明显混有多方的内容，建议在文档末尾额外加入一小节 `归因说明`，说明你如何区分：
